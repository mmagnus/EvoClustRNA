...
def get_parser():
    ...
    parser.add_argument('-o',"--output_fn", help="output csv file")
...
def calc_evo_rmsd(targetfn, target_name_alignment, files, mapping, rna_alignment_fn, group_name='', output_fn=None):
    ...
    rnastruc = mapping.strip().split('|') # 'target:rp14_farna_eloop_nol2fixed_cst|X:X'
    ...
    if output_fn:
        df.to_csv(output_fn)
    df.sort(columns=('rmsd'), inplace=True)
    #df.reset_index(drop=True)
    df.reset_index(drop=True, inplace=True)
    df.plot(df.index, y='rmsd')
...
if __name__ == '__main__':
    ...
    df = calc_evo_rmsd(opts.target, opts.target_name, opts.files, opts.mapping, opts.rna_alignment_fn, opts.group_name, opts.output_fn)
    print df
